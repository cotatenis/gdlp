...
class GCSPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if item_name == 'GDLPItem':
            raw_item = item
            sizes_and_stock = raw_item.get("sizes_and_stock", None)
            if not sizes_and_stock:
                #algum problema de layout da página ocasionou que não foi possível coletar esse campo.
                raise DropItem()
            product_name = raw_item.get("product")
            if 'SANDALIA' in product_name:
                raise DropItem()
...
class GDLPImagePipeline(ImagesPipeline):
    ...
    def thumb_path(self, request, thumb_id, response=None, info=None):
        sku = request.meta['sku']
        image_name = f"{request.url.split('/')[-1]}"
